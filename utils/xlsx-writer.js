...
function buildXlsx(records, columns, helpers) {
  ...
  var streamToFile = helpers.streamToFile;     // {fs, filePath} — 流式写入到文件

  if (!streamToFile && !imageBytesMap && !getImageBytes) throw new Error('imageBytesMap or getImageBytes is required');
  if (streamToFile && !getImageBytes) throw new Error('getImageBytes is required when streamToFile is set');

  // 收集图片元数据（流式模式不预读字节）
  var imageEntries = []; // {rId, path, bytes?, ext, idx, recordIdx}
  var imgIdx = 0;
  ...
  if (streamToFile) {
    zip.openFile(streamToFile.fs, streamToFile.filePath);
  }
  ...
  if (imageEntries.length > 0) {
    ...
    for (var i = 0; i < imageEntries.length; i++) {
      ...
      var imgBytes;
      if (streamToFile) {
        // 流式模式：单张读取，写入后 zip.addFile 立即释放内存
        try {
          imgBytes = getImageBytes(ie.path);
        } catch (e) {
          console.warn('[XlsxWriter] 图片读取失败（跳过）:', ie.path, e);
          continue;
        }
        if (!imgBytes || !imgBytes.length) {
          console.warn('[XlsxWriter] 图片空字节（跳过）:', ie.path);
          continue;
        }
      } else {
        imgBytes = ie.bytes;
      }
      zip.addFile('xl/media/image' + ie.idx + '.' + ie.ext, imgBytes);
    }
  }

  if (streamToFile) {
    zip.closeFile();
    return streamToFile.filePath;
  } else {
    return zip.end();
  }
}

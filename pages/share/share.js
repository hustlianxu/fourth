...
Page({
  ...
  onShareAppMessage() {
    ...
    var title = '水印照片分享 · ' + (record ? (record.customName || record.templateName) : '照片');
    return {
      title: title,
      path: '/pages/detail/detail?id=' + this.data.recordId + '&_authPerm=' + perm,
      imageUrl: record ? record.imagePath : ''
    };
  },

  // ===== 高级选项：手动输入 openid（收起状态，供有需要的用户） =====

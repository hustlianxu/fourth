...
const OAuthInputSchema = {
    action: z.enum(['authorize', 'revoke', 'status']).describe('The OAuth action to perform: "authorize" starts device flow, "revoke" removes stored token, "status" checks authorization status'),
    scope: z.string().optional().describe('Space-separated list of OAuth scopes to request (e.g., "contact:user.base:readonly mail:mail:readonly"). Used with "authorize" action.'),
    user_open_id: z.string().optional().describe('The user open_id for "revoke" or "status" action. For "authorize", this will be discovered automatically.'),
};
/**
 * Handle 'authorize' action - start OAuth device flow.
 */
async function handleAuthorize(scope, appId, appSecret, brand) {
    ...
    try {
        ...
        // Save the token
        await saveTokenFromDeviceFlow({ appId, appSecret, userOpenId, domain: brand }, {
 ...
 */
async function handleRevoke(userOpenId, appId) {
    if (!userOpenId) {
 ...
 */
async function handleStatus(userOpenId, appId, appSecret, brand) {
    if (!userOpenId) {
        // List all stored tokens for this app
        const tokens = await listStoredTokens(appId);
        if (tokens.length === 0) {
 ...
 */
export function registerOAuthTool(registry) {
    registry.register({
        name: 'feishu_oauth',
        description: [

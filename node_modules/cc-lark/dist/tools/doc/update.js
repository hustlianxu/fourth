...
const updateDocSchema = {
    ...
    mode: updateModeSchema.describe('Update mode: overwrite, append, replace_range, replace_all, insert_before, insert_after, delete_range (required)'),
...
function validateUpdateDocParams(p) {
    ...
    if (selectionModes.includes(mode)) {
        ...
        if ((hasEllipsis && hasTitle) || (!hasEllipsis && !hasTitle)) {
            throw new Error('update-doc: For modes replace_range/insert_before/insert_after/delete_range, ' +
 ...
 */
export function registerUpdateDocTool(registry) {
    registry.register({
...
// Handler
// ---------------------------------------------------------------------------
async function handleUpdateDoc(args, context) {
    const p = args;
    ...
    try {
        ...
        const mcpArgs = {};
        if (p.doc_id)
            mcpArgs.doc_id = p.doc_id;
        if (p.markdown)
            mcpArgs.markdown = p.markdown;
        if (p.mode)
            mcpArgs.mode = p.mode;
        if (p.selection_with_ellipsis)
            mcpArgs.selection_with_ellipsis = p.selection_with_ellipsis;
        if (p.selection_by_title)
            mcpArgs.selection_by_title = p.selection_by_title;
        if (p.new_title)
            mcpArgs.new_title = p.new_title;
        if (p.task_id)

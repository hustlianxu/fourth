...
function normalizeSearchResultTimeFields(value) {
    if (Array.isArray(value)) {
        return value.map((item) => normalizeSearchResultTimeFields(item));
    }
...
async function getAccessToken(context) {
    ...
    if (tokens.length === 0)
        return jsonError('No user authorization found.');
    ...
    try {
        ...
        if (err instanceof NeedAuthorizationError)
            return jsonError('User authorization expired.');
        throw err;
    }
}
export function registerSearchDocWikiTool(registry) {
    registry.register({
        ...
        handler: async (args, context) => {
            ...
            if (res.code !== undefined && res.code !== 0) {
                return jsonError(`API Error: code=${res.code}, msg=${res.msg}`);
            }
            const data = res.data || {};

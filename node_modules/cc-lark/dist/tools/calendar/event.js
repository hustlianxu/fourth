...
function parseTimeToTimestamp(input) {
    try {
        ...
        const utcDate = new Date(Date.UTC(parseInt(year), parseInt(month) - 1, parseInt(day), parseInt(hour) - 8, // Beijing time - 8 hours = UTC
...
// Tool registration
// ---------------------------------------------------------------------------
export function registerCalendarEventTool(registry) {
    registry.register({
...
async function resolveCalendarId(calendarId, larkClient, accessToken) {
    if (calendarId)
        return calendarId;
...
function normalizeEventTimeFields(event) {
    ...
    if (startTime && typeof startTime === 'object' && 'timestamp' in startTime) {
        ...
        const iso = unixTimestampToISO8601(ts);
        if (iso) {
...
async function handleCreate(args, context) {
    ...
    if (!startTs || !endTs) {
        return jsonError("Invalid time format. Must use ISO 8601 with timezone, e.g., '2024-01-01T00:00:00+08:00'", { received_start: p.start_time, received_end: p.end_time });
    }
    ...
    if (p.location_name)
        eventData.location = { name: p.location_name };
...
async function handleList(args, context) {
    ...
    const events = (data?.items ?? []).map((item) => normalizeEventTimeFields(item));
    return json({
        events,
...
async function handleGet(args, context) {
    const p = args;
    const { larkClient } = context;
    if (!p.event_id) {
...
async function handlePatch(args, context) {
    const p = args;
    const { larkClient } = context;
    if (!p.event_id) {
        return jsonError('event_id is required');

...
export function registerTaskTool(registry) {
    registry.register({
        ...
        handler: async (args, context) => {
            ...
            if (p.due_timestamp) {
                ...
                if (!ts)
                    return jsonError("Invalid due_timestamp format. Use ISO 8601 with timezone, e.g., '2024-01-01T00:00:00+08:00'");
                taskData.due = { timestamp: ts, is_all_day: false };
            }
            if (p.start_timestamp) {
                const ts = parseTimeToTimestampMs(p.start_timestamp);
                if (!ts)
                    return jsonError("Invalid start_timestamp format. Use ISO 8601 with timezone.");
    ...
    registry.register({
        ...
        handler: async (args, context) => {
            ...
            const Lark = await import('@larksuiteoapi/node-sdk');
            const opts = Lark.withUserAccessToken(accessToken);
            const res = await larkClient.sdk.task.v2.task.get({ path: { task_guid: p.task_guid }, params: { user_id_type: 'open_id' } }, opts);
            assertLarkOk(res);
            return json({ task: res.data?.task });
        },
    });
    registry.register({
        ...
        handler: async (args, context) => {
            ...
            const Lark = await import('@larksuiteoapi/node-sdk');
            const opts = Lark.withUserAccessToken(accessToken);
            const res = await larkClient.sdk.task.v2.task.list({ params: { page_size: p.page_size, page_token: p.page_token, completed: p.completed, user_id_type: 'open_id' } }, opts);
            assertLarkOk(res);
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const data = res.data;
            return json({
    ...
    registry.register({
        ...
        handler: async (args, context) => {
            ...
            if (p.due_timestamp) {
                const ts = parseTimeToTimestampMs(p.due_timestamp);
                if (!ts)
                    return jsonError("Invalid due_timestamp format.");
            ...
            if (p.start_timestamp) {
                const ts = parseTimeToTimestampMs(p.start_timestamp);
                if (!ts)
                    return jsonError("Invalid start_timestamp format.");
            ...
            const res = await larkClient.sdk.task.v2.task.patch({ path: { task_guid: p.task_guid }, data: { task: updateData, update_fields: updateFields }, params: { user_id_type: 'open_id' } }, opts);
            assertLarkOk(res);
            return json({ task: res.data?.task });
        },
    });

 ...
 */
export function convertMessageContent(raw, msgType, _ctx) {
    if (!raw)
        return '';
    try {
 ...
 */
function extractPostContent(parsed) {
    const sections = [];
...
function extractLineText(line) {
    ...
    for (const element of line) {
        ...
        if (el.text_run) {
            ...
            if (tr.content)
                parts.push(String(tr.content));
        }
        else if (el.mention_run) {
            const mr = el.mention_run;
            if (mr.text)
                parts.push(String(mr.text));
        }
        else if (el.equation_run) {
            const er = el.equation_run;
            if (er.content)
                parts.push(`$${er.content}$`);
        }
        else if (el.text) {
 ...
 */
export async function formatMessageList(items, accountId, nameResolver, batchResolver) {

...
import { json, jsonError, getCachedUserName, setCachedUserNames } from './helpers.js';
...
const timeRangeShape = {
    relative_time: z
        .string()
        .optional()
        .describe('Relative time range: today / yesterday / day_before_yesterday / this_week / last_week / this_month / last_month / last_{N}_{unit} (unit: minutes/hours/days). Mutually exclusive with start_time/end_time.'),
    start_time: z
        .string()
        .optional()
 ...
 */
async function resolveP2PChatId(sdk, openId, accessToken, logInfo) {
 ...
 */
async function batchResolveUserNames(sdk, openIds, accessToken) {
 ...
 */
export function registerImMessageReadTools(registry) {
...
function registerGetMessages(registry) {
    registry.register({
        ...
        handler: async (args, context) => {
            ...
            const nameResolver = (id) => getCachedUserName(id);
            const batchResolver = async (ids) => {
...
function registerGetThreadMessages(registry) {
    registry.register({
...
function registerSearchMessages(registry) {
    registry.register({
        ...
        handler: async (args, context) => {
            ...
            if (p.sender_ids?.length)
                searchData.from_ids = p.sender_ids;
            if (p.chat_id)
                searchData.chat_ids = [p.chat_id];
            if (p.mention_ids?.length)
                searchData.at_chatter_ids = p.mention_ids;
            if (p.message_type)
                searchData.message_type = p.message_type;
            if (p.sender_type && p.sender_type !== 'all')
                searchData.from_type = p.sender_type;
            if (p.chat_type)
                searchData.chat_type = p.chat_type === 'group' ? 'group_chat' : 'p2p_chat';
            logInfo(`search: query="${p.query ?? ''}", page_size=${p.page_size ?? 50}`);
            const Lark = await import('@larksuiteoapi/node-sdk');
            const opts = Lark.withUserAccessToken(accessToken);
            // Step 1: Search for message IDs
            const searchRes = await context.larkClient.sdk.search.message.create({
                data: searchData,
                params: {
            ...
            const p2pTargetIds = [...new Set([...chatMap.values()].map((c) => c.p2p_target_id).filter(Boolean))];
...
async function fetchChatContexts(sdk, accessToken, chatIds, logInfo) {
    const map = new Map();
    if (chatIds.length === 0)
        return map;
    try {

...
export function dateTimeToSeconds(datetime) {
    ...
    if (isNaN(d.getTime())) {
        throw new Error(`Unable to parse ISO 8601 time: "${datetime}". Example format: 2026-02-27T14:30:00+08:00`);
    }
...
export function parseTimeRange(input) {
    ...
    switch (input) {
        ...
        default: {
            ...
            if (!match) {
                throw new Error(`Unsupported relative_time format: "${input}". ` +
...
export function parseTimeToTimestamp(input) {
    try {
        ...
        const utcDate = new Date(Date.UTC(parseInt(year), parseInt(month) - 1, parseInt(day), parseInt(hour) - 8, // Subtract 8 hours from Beijing time to get UTC

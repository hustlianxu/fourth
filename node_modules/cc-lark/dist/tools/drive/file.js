...
export function registerDriveFileTool(registry) {
    // List files
    registry.register({
    ...
    registry.register({
        ...
        handler: async (args, context) => {
            ...
            const Lark = await import('@larksuiteoapi/node-sdk');
            const opts = Lark.withUserAccessToken(accessToken);
            const res = await larkClient.sdk.drive.meta.batchQuery({ data: { request_docs: p.request_docs } }, opts);
            assertLarkOk(res);
    ...
    registry.register({
        ...
        handler: async (args, context) => {
            ...
            const opts = Lark.withUserAccessToken(accessToken);
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const data = { name: p.name, type: p.type };
            if (p.folder_token)
                data.folder_token = p.folder_token;
            const res = await larkClient.sdk.drive.file.copy({ path: { file_token: p.file_token }, data }, opts);
            assertLarkOk(res);
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    registry.register({
        ...
        handler: async (args, context) => {
            ...
            const Lark = await import('@larksuiteoapi/node-sdk');
            const opts = Lark.withUserAccessToken(accessToken);
            const res = await larkClient.sdk.drive.file.move({ path: { file_token: p.file_token }, data: { type: p.type, folder_token: p.folder_token } }, opts);
            assertLarkOk(res);
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const data = res.data;
            return json({
    ...
    registry.register({
        ...
        handler: async (args, context) => {
            ...
            const Lark = await import('@larksuiteoapi/node-sdk');
            const opts = Lark.withUserAccessToken(accessToken);
            const res = await larkClient.sdk.drive.file.delete({ path: { file_token: p.file_token }, params: { type: p.type } }, opts);
            assertLarkOk(res);
            const data = res.data;
            return json({
                success: true,
